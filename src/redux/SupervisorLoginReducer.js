...
export function SupervisorLoginAction(payload) {
  // MAKE SURE redux-thunk is installed.
  return async (dispatch) => {
    // WE HV TO CALL THE SPRINT1 / SPRING BOOT
    try {
      const url = "http://localhost:8080/api/supervisor/login";
...
// REDUCER LOGIC
export function SupervisorLoginReducer(state = initState, action) {
  switch (action.type) {
    case LOGIN_ACTION:
